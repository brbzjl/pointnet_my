...
DATA_PATH = os.path.join(ROOT_DIR, 'data', 'Stanford3dDataset')
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names_street.txt'))]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}
g_class2color = {'road':	[128, 64,128],
                 'sidewalk':	[244, 35,232],
                 'building':	[ 70, 70, 70],
                 'wall':        [102,102,156],
                 'fence':      [190,153,153],
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    for idx in range(len(xbeg_list)): 
       xbeg = xbeg_list[idx]
       ybeg = ybeg_list[idx]
       xcond = (data[:,0]<=xbeg+block_size) & (data[:,0]>=xbeg)# inside of this block
