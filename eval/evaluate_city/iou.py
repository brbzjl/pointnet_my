...
def evaluatePair(predictionImg, groundTruthImg, confMatrix, instanceStats, perImageStats, args):
    ...
    if (CSUPPORT):
        ...
        for (groundTruthImgPixel,predictionImgPixel) in izip(np.ndarray.flatten(groundTruthImg),np.ndarray.flatten(predictionImg)):
...
def get_iou(pred,gt):
    global args
    args.quiet = True
    return evaluateImgLists(pred, gt, args)

# The main method
# def main():
#     global args
#
#     a = np.reshape([0,1,2,3,4,5,6],(1,1,-1))
#     b = np.reshape([0,2,2,3,3,4,4],(1,1,-1))
#     print (np.shape(a))
#     evaluateImgLists(a, b, args)
#
#
